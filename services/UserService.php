    ...
    public function editUser(User $updatedUser, bool $isImmediate): bool{
        try{
            ...
            if($currentUser->getEmail() != $updatedUser->getEmail()){
                ...
                if($stash->generateFromUser($updatedUser, $nextStashId) && $editEmailKey->fillForId($nextStashId, $currentUser->getEmail(), $updatedUser->getEmail())){
                    if($userEditsService->createEditStashAndEditEmailKey($stash, $editEmailKey)) {
                        ...
                        if(MailService::getInstance()->sendMail($currentUser->getEmail(), $subject, $oldBody) &&
