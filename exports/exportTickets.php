...
if($_POST){
    ...
    if(
        (!empty($_POST["id"]) || !empty($_POST["ticketType"]) || !empty($_POST["eventId"]) || !empty($_POST["eventType"])
            || !empty($_POST["price"]) || !empty($_POST["inStock"])) && !empty($_POST["format"])
    ){
        ...
        $data = $ticketService->getColumns($args);
    }
    else{
